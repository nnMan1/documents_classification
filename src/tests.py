...
def find_outliers(distance_matrix):    
    classifier = LocalOutlierFactor(n_neighbors=5, algorithm='brute', metric='precomputed', novelty=True, contamination=0.19, n_jobs=-1)
...
random_sample = random.sample(data['train'], 700)

distance = Distance([d.text  for d in random_sample], min_similarity=0.17)
